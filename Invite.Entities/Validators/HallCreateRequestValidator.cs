...
public class HallCreateRequestValidator : AbstractValidator<HallCreateRequest>
{
    public HallCreateRequestValidator()
    {
        ...
        RuleFor(x => x.PriceInWeekend)
            .NotEmpty().WithMessage("Preço durante o fim de semana é um campo obrigatório!");

        RuleFor(x => x.PriceInWeek)
            .NotEmpty().WithMessage("Preço durante a semana é um campo obrigatório!");
    }
}
